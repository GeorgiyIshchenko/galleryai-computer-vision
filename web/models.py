...
class Photo(models.Model):
    ...
    def __str__(self):
        return f'{self.pk} | {str(self.user).split("@")[0]} | {str(self.image).split("/")[-1]}'
...
class Project(models.Model):
    ...
    def get_random_photo_n(self):
        trained_match = self.photos.filter(Q(meta__match=False) & Q(meta__is_ai_tag=False))
        if trained_match.count():
            random_photo = trained_match.order_by('?')[0]
            return random_photo
    ...
    def get_zip_url_match(self):
        return reverse('web:project_zip_m', kwargs={'pk': self.id})

    def get_zip_url_not_match(self):
        return reverse('web:project_zip_nm', kwargs={'pk': self.id})

    def get_match(self):
        return self.photos.filter(meta__match=True)

    def get_not_match(self):
        return self.photos.filter(meta__match=False)

    def get_match_photos_count(self):
        return self.photos.filter(meta__match=True).count()

    def get_not_match_photos_count(self):
        return self.photos.filter(meta__match=False).count()
